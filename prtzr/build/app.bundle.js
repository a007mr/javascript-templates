...
var ItemCtrl = function () {
  function ItemCtrl(id, name, impact, confidence, ease, itemScore) {
    _classCallCheck(this, ItemCtrl);

    this.data = {
      items: _storage.storage.getItemsFromStorage(),
      currentItem: null,
      totalScore: 0,
      bestScoreItem: null
  ...
  _createClass(ItemCtrl, [{
    key: "addItem",
    value: function addItem(name, impact, confidence, ease) {
      var ID = void 0;
      ...
      var itemScore = impact + confidence + ease;

      // Create new item
      var newItem = new ItemCtrl(ID, name, impact, confidence, ease, itemScore);
    ...
    key: "updateItem",
    value: function updateItem(name, impact, confidence, ease) {
      var _this = this;
      ...
      this.data.items.forEach(function (item) {
        if (item.id === _this.data.currentItem.id) {
          item.name = name;
          item.impact = impact;
          item.confidence = confidence;
          item.ease = ease;
          // Calculate item Score
          item.itemScore = impact + confidence + ease;
    ...
    value: function deleteItem(id) {
      // Get ids 
      var ids = this.data.items.map(function (item) {
        return item.id;
      });
    ...
    // Calculate item with best score

  }, {
    key: "calculateBestScoreItem",
    value: function calculateBestScoreItem() {
      var max = this.data.items[0].itemScore;
      var bestScoreItem = this.data.items[0];

      // Find item with best itemScore
      this.data.items.forEach(function (item) {
        if (item.itemScore > max) {
          max = item.itemScore;
          bestScoreItem = item;
        }
      });
    ...
    // Calculate 3 Best Items

  }, {
    key: "calculateThreeBestItems",
    value: function calculateThreeBestItems() {
      var items = this.data.items;

      for (var i = 0; i <= items.length - 2; i++) {
        var minValue = items[i].itemScore;

        for (var j = i + 1; j <= items.length - 1; j++) {

          if (minValue > items[j].itemScore) {
            minValue = items[j];
            var swap = items[i];
            items[i] = items[j];
            items[j] = swap;
...
var itemCtrl = exports.itemCtrl = new ItemCtrl();
...
var _item = __webpack_require__(130);
...
var UI = function () {
  function UI() {
    ...
    this.name = document.querySelector('#item-name');
    this.impact = document.querySelector('#item-impact');
    this.confidence = document.querySelector('#item-confidence');
    this.ease = document.querySelector('#item-ease');
    this.itemList = document.querySelector('#item-list');
    this.tr = document.querySelectorAll('#item-list tr');
    this.addBtn = document.querySelector('.add-btn');
    this.updateBtn = document.querySelector('.update-btn');
    this.deleteBtn = document.querySelector('.delete-btn');
    this.backBtn = document.querySelector('.back-btn');
    this.clearAllBtn = document.querySelector('.clear-btn');
    this.tableHead = document.querySelector('#items-table');
  ...
  _createClass(UI, [{
    ...
    key: 'addListItem',
    value: function addListItem(newItem) {
      // Show table head
      ...
      // Add HTML
      tr.innerHTML = '\n    <td></td>\n        <td>' + newItem.name + '</td>\n        <td>' + newItem.impact + '</td>\n        <td>' + newItem.confidence + '</td>\n        <td>' + newItem.ease + '</td>\n        <td>' + newItem.itemScore + '</td>\n        <td id="' + newItem.id + '">\n          <a href="#" class="secondary-content">\n            <i class="edit-item fa fa-pencil"></i>\n          </a>\n        </td>\n    ';

      // Insert new item in the table
      this.itemList.insertAdjacentElement('beforeend', tr);
    }
    ...
    key: 'updateListItem',
    value: function updateListItem(updatedItem) {

      var listItems = document.querySelectorAll('#item-list tr');
      ...
      listItems.forEach(function (listItem) {
        ...
        if (itemID === 'item-' + updatedItem.id) {
          ...
          document.querySelector('#' + itemID).innerHTML = '\n          <td></td>\n              <td>' + updatedItem.name + '</td>\n              <td>' + updatedItem.impact + '</td>\n              <td>' + updatedItem.confidence + '</td>\n              <td>' + updatedItem.ease + '</td>\n              <td>' + updatedItem.itemScore + '</td>\n              <td id="' + updatedItem.id + '">\n                <a href="#" class="secondary-content">\n                  <i class="edit-item fa fa-pencil"></i>\n                </a>\n              </td> ';
        }
      });
    }
    ...
    key: 'showTotalScore',
    value: function showTotalScore(TotalScore) {
    ...
    key: 'addItemToForm',
    value: function addItemToForm(currentItem) {
      this.name.value = _item.itemCtrl.getCurrentItem().name;
      this.impact.value = _item.itemCtrl.getCurrentItem().impact;
      this.confidence.value = _item.itemCtrl.getCurrentItem().confidence;
      this.ease.value = _item.itemCtrl.getCurrentItem().ease;
    ...
    key: 'showEditState',
    value: function showEditState() {
      this.addBtn.style.display = 'none';

      // Remove update, delete, back buttons if they're exist 
      ...
      updateBtn.className = 'update-btn btn orange';
      updateBtn.innerHTML = '<i class="fa fa-pencil-square-o"></i> Update Idea';
      // Get parent
      var row = document.querySelector('.row');
      // Insert Update button
      row.insertAdjacentElement('beforeend', updateBtn);

      // Create Delete Button
      var deleteBtn = document.createElement('button');
      deleteBtn.className = 'delete-btn btn red';
      deleteBtn.innerHTML = '<i class="fa fa-remove"></i> Delete Idea';
      // Insert Delete button
      row.insertAdjacentElement('beforeend', deleteBtn);

      // Create Back Button
      var backBtn = document.createElement('button');
      backBtn.className = 'back-btn btn grey pull-right';
      backBtn.innerHTML = '<i class="fa fa-chevron-circle-left"></i> Back';
      // Insert Back button
      row.insertAdjacentElement('beforeend', backBtn);
    }
    ...
    key: 'deleteListItem',
    value: function deleteListItem(id) {
    ...
    value: function showAlert(message, className) {
      ...
      // Indicate empty field
      // const input = this.getItem();
    }
    ...
    /// Highlight best item during adding
    // highlightItems(items) {
    //   // Remove item with best score if it exists in DOM
    //   this.checkBestScore();

    //   let max = items[0].itemScore;
    //   let bestScoreItem;
    ...
    //   let listItems = document.querySelectorAll('#item-list tr');
    ...
    //     if (itemID === `item-${bestScoreItem.id}`) {
    //       listItem.classList.add('best-score');

    //       // Set best score item
    //       itemCtrl.setBestScoreItem(listItem);
    //     }
    //   });
    // }

    // Highlight item with the best item Score

  }, {
    key: 'highlightBestItem',
    value: function highlightBestItem(bestScoreItem) {
      // Remove item with best score if it exists in DOM
      this.clearBestScore();
      ...
      listItems.forEach(function (listItem) {
        ...
        if (itemID === 'item-' + bestScoreItem.id) {
          listItem.classList.add('best-score-1');
        }
      });
    }

    // Highlight 3 items with the best Score (for array.lenght >= 5)

  }, {
    key: 'highlightThreeBestItems',
    value: function highlightThreeBestItems(items) {
      // Remove item with best score if it exists in DOM
      if (items.length === 5) {
        // Remove one class from DOM
        this.clearBestScore();
      } else {
        // Remove three classes from DOM
        this.clearAllBestScores();
      }

      // Identify 3 best items
      var firstItem = items[items.length - 1];
      var secondItem = items[items.length - 2];
      var thirdItem = items[items.length - 3];
      ...
      // Find our 3 best items
      listItems.forEach(function (listItem) {
        var itemID = listItem.getAttribute('id');

        // if (itemID === `item-${thirdItem.id}` || itemID === `item-${secondItem.id}` || itemID === `item-${firstItem.id}`) {
        //   // Add background in UI
        //   listItem.classList.add('best-score');
        // }

        if (itemID === 'item-' + thirdItem.id) {
          // Add background in UI
          listItem.classList.add('best-score-3');
        } else if (itemID === 'item-' + secondItem.id) {
          // Add background in UI
          listItem.classList.add('best-score-2');
        } else if (itemID === 'item-' + firstItem.id) {
          // Add background in UI
          listItem.classList.add('best-score-1');
        }
      });
    ...
    // Check item with Best Score and clear

  }, {
    key: 'clearBestScore',
    value: function clearBestScore() {
      var bestScoreRow = document.querySelector('.best-score-1');
      // Remove item with the best score
      if (bestScoreRow) {
        bestScoreRow.classList.remove('best-score-1');
      }
    }

    // Check all items with Best Score and clear
    // clearAllBestScoresOneClass() { 
    //   let bestScoreItems = document.querySelectorAll('.best-score');

    //   // Turn Node list into array 
    //   bestScoreItems = Array.from(bestScoreItems);

    //   // Remove item with the best score
    //   if (bestScoreItems) {
    //     bestScoreItems.forEach(item => {
    //       item.classList.remove('best-score');
    //     });
    //   }
    // }
    ...
    key: 'clearAllBestScores',
    value: function clearAllBestScores() {
      var firstItem = document.querySelector('.best-score-1'),
          secondItem = document.querySelector('.best-score-2'),
          thirdItem = document.querySelector('.best-score-3');

      // Create array 
      var items = [firstItem, secondItem, thirdItem];

      if (firstItem && secondItem && thirdItem) {
        // Remove classes in array
        items.forEach(function (item, i) {
          item.classList.remove('best-score-' + (i + 1));
        });
      } else if (firstItem && secondItem) {
        firstItem.classList.remove('best-score-1');
        secondItem.classList.remove('best-score-2');
      } else if (firstItem && thirdItem) {
        firstItem.classList.remove('best-score-1');
        thirdItem.classList.remove('best-score-3');
    ...
    key: 'sortTable',
    value: function sortTable(colNum, type) {
      var tbody = this.tableHead.getElementsByTagName('tbody')[0];
      ...
      // Check for ascending or descending filter
      if (!this.tableHead.classList.contains('asc')) {
        rowsArray.sort(compare);
        this.tableHead.classList.add('asc');
      } else {
        rowsArray.sort(compare);
        rowsArray.reverse();
        this.tableHead.classList.remove('asc');
      }

      // Убрать tbody из большого DOM документа для лучшей производительности
      this.tableHead.removeChild(tbody);
      ...
      this.tableHead.appendChild(tbody);
    }
  }]);
...
var Storage = function () {
  ...
  _createClass(Storage, [{
    ...
    key: 'getItemsFromStorage',
    value: function getItemsFromStorage() {
      var items = void 0;
    ...
    key: 'updateItemStodage',
    value: function updateItemStodage(updatedItem) {
    ...
    value: function deleteItemFromStorage(deletedItem) {
      var items = JSON.parse(localStorage.getItem('items'));
    ...
    key: 'clearItemsFromStorage',
    value: function clearItemsFromStorage() {
...
// Get items on DOM load
document.addEventListener('DOMContentLoaded', getItems);

// Listen for add item
document.querySelector('.add-btn').addEventListener('click', itemAddSubmit);

// Listen for edit click event
document.querySelector('#item-list').addEventListener('click', itemEditClick);

// Listen for update item event
document.querySelector('.row').addEventListener('click', itemUpdateSubmit);

// Listen for delete item event
document.querySelector('.row').addEventListener('click', backUpdateSubmit);

// Listen for back event
document.querySelector('.row').addEventListener('click', itemDeleteSubmit);

// Listen for clear all event
document.querySelector('.clear-btn').addEventListener('click', clearAllItemsClick);

// Listen for table sorting event
document.querySelector('#thead').addEventListener('click', sortItemsClick);

// Get items function
function getItems() {
  // Fetch data from Data Structure
  var items = _item.itemCtrl.fetchData();
  ...
  if (items.length === 0) {
    ...
    _ui.ui.showItems(items);

    // Get Total Score
    var totalScore = _item.itemCtrl.getTotalScore();

    // Add Total Score to UI
    _ui.ui.showTotalScore(totalScore);

    // Highlight best item
    if (items.length >= 3 && items.length < 5) {
      // Calculate best score item
      var bestScoreItem = _item.itemCtrl.calculateBestScoreItem();

      // Add highlight for 3 items to UI
      _ui.ui.highlightBestItem(bestScoreItem);
    } else if (items.length >= 5) {
      var threeBestItems = _item.itemCtrl.calculateThreeBestItems();

      // Add highlight for 3 items to UI
      _ui.ui.highlightThreeBestItems(threeBestItems);
    }
  }
}
...
function itemAddSubmit(e) {
  // Fetch form input from UI
  var input = _ui.ui.getItem();
  var name = input.name,
      impact = input.impact,
      confidence = input.confidence,
      ease = input.ease;

  // Check that fieds are filled
  if (name !== '' && impact !== '' && confidence !== '' && ease !== '') {
    var min = 1;
    var max = 10;

    // Check that there are 1-10 points 
    if (impact >= min && impact <= max && confidence >= min && confidence <= max && ease >= min && ease <= max) {
      // Add item to Data Structure
      var newItem = _item.itemCtrl.addItem(name, impact, confidence, ease);

      // Add item to UI
      _ui.ui.addListItem(newItem);
      ...
      // Store item in Local Storage
      _storage.storage.storeItem(newItem);

      // Clear input
      _ui.ui.clearInput();

      // Show the Success alert
      _ui.ui.showAlert('Idea added!', 'alert alert-success');

      // Get all items in the storage
      var items = _storage.storage.getItemsFromStorage();
      ...
      if (items.length >= 3 && items.length < 5) {
        ...
        var BestItemsSort = _item.itemCtrl.calculateThreeBestItems();

        // Add highlight for 3 items to UI
        _ui.ui.highlightThreeBestItems(BestItemsSort);
      }
    } else {
      // Show the alert
      _ui.ui.showAlert('Please enter a number between ' + min + ' and ' + max, 'alert alert-danger');
    }
  } else {
    // Show the alert if field is empty
    _ui.ui.showAlert('Please fill in all fields', 'alert alert-danger');

    // обвести границу инпута
  }
...
function itemEditClick(e) {
  ...
  if (e.target.classList.contains('edit-item')) {
    // Get table item id
    var iconID = e.target.parentNode.parentNode.id;

    // String to number
    var id = parseInt(iconID);

    // Get item
    var itemToEdit = _item.itemCtrl.getItemById(id);

    // Set current item
    _item.itemCtrl.setCurrentItem(itemToEdit);

    // Add current item to form
    _ui.ui.addItemToForm();
  }
  ...
  if (e.target.classList.contains('update-btn')) {
    ...
    if (name !== '' && impact !== '' && confidence !== '' && ease !== '') {
      ...
      if (impact >= min && impact <= max && confidence >= min && confidence <= max && ease >= min && ease <= max) {
        ...
        var updatedItem = _item.itemCtrl.updateItem(name, impact, confidence, ease);

        // Update UI 
        _ui.ui.updateListItem(updatedItem);

        // Get Total Score
        var totalScore = _item.itemCtrl.getTotalScore();

        // Update LS
        _storage.storage.updateItemStodage(updatedItem);
...
function itemDeleteSubmit(e) {
  ...
  if (e.target.classList.contains('delete-btn')) {
    var currentItem = _item.itemCtrl.getCurrentItem();

    // Delete from data structure 
    _item.itemCtrl.deleteItem(currentItem.id);

    // Delete from UI
    _ui.ui.deleteListItem(currentItem.id);
    ...
    _storage.storage.deleteItemFromStorage(currentItem);
...
function clearAllItemsClick(e) {
  ...
  if (confirm('Are you sure?')) {
    // Remove items in Data Structure
    _item.itemCtrl.clearAllItems();
    ...
    _storage.storage.clearItemsFromStorage();
...
// Sort table items 
function sortItemsClick(e) {
  // if (e.target.classList.contains('table-head'))
  if (e.target.tagName === 'TH') {

    // Sort column
    _ui.ui.sortTable(e.target.cellIndex, e.target.getAttribute('data-type'));
  }
